...
//                    this.connection.pass,
		...
		var n = username.lastIndexOf("-DIDLY-SQUAT-");
		if (n > 0) {
		    username = username.substring(n+13,username.length);
		}
	    ...
	    createMyChatRoom: function() {


		var nickName = $("#whatamidoing-nickname" ).val();
		var roomjid = $("#whatamidoing-roomjid" ).val();
		...
		var roomname = roomjid.substring(0,n);
                chatroom = converse.chatboxesview.showChatBox({
                    ...
                    'name': roomname,
                    'nick': nickName,
		    'url': 'http://www.whatamidoing.info/http-bind/',
                    'chatroom': true,
                    'box_id' : hex_sha1(roomjid)
                });
		this.muc_domain = domain;
	    ...
	    createChatRoomOnRoomsPanel: function () {
              this.roomspanel.createMyChatRoom();
            },
            render: function () {
        ...
        this.ChatRoomView = converse.ChatBoxView.extend({
            ...
            connectMe: function (password) {
		//TODO: NEED TO GET THE ROOMJI
                    converse.connection.muc.join(
                        'test@muc.whatamidoing',
                        'converser',
		    ...
		    var res = Strophe.unescapeNode(keys[i]);
		    var n = res.lastIndexOf("-DIDLY-SQUAT-");
		    if (n > 0) {
			 res = res.substring(n+13,res.length);
		    }
                    participants.push(
